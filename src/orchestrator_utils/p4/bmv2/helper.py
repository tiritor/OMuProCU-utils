...
from ..v1 import p4runtime_pb2
from ..config.v1 import p4info_pb2

from ..bmv2.convert import encode
...
class P4InfoHelper(object):
        ...
        if match_fields:
            table_entry.match.extend([
                self.get_match_field_pb(
                    table_name.decode("utf-8") if isinstance(table_name, bytes) else table_name, 
                    match_field_name.decode("utf-8") if isinstance(match_field_name, bytes) else match_field_name, 
    ...
    def build_clone_session_entry(self, clone_session_id, replicas, packet_length_bytes=0):
        ...
        for replica in replicas:
            ...
            replica_.egress_port = replica['egress_port']
            replica_.instance = replica['instance']
            clone_entry.clone_session_entry.replicas.extend([replica_])
        return clone_entry
