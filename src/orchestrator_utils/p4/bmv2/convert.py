...
def encode_mac_address(mac_address_string):
    mac_address_string = mac_address_string.replace(':', '')
    return codecs.decode(mac_address_string.replace(':', ''), "hex")
...
def encode_number(number, bit_width):
    ...
    number_str = '%x' % number
    return codecs.decode(('0' * (byte_len * 2 - len(number_str)) + number_str), 'hex')
